...
Fixpoint ceval_step (st : state) (c : com) (continuation: list (state * com)) (i : nat)
  ...
  | S i' => 
      match c with
        ...
        | <{ while b1 do c1 end }> =>
            if (beval st b1)
              then LETOPT st' & cont' <== ceval_step st c1 continuation i' IN
        ...
        | <{ b1 -> c1 }> => 
            if (beval st b1)
              then ceval_step st c1 continuation i'
                    ...
                    | nil => Fail
                    | (st', c') :: cont' => ceval_step st' <{ c' ; c}> cont' i'
                  end
    end
  end.
...
induction i1 as [|i1']; intros i2 st st' c cont cont' Hle Hceval.
  ...
  - (* i1 = S i1' *)
    ...
    + (* ; *)
      ...
      ---(* success = success *)
        induction s. apply (IHi1' i2') in Heqst1'o; try assumption.
    ...
    + (* !! *)
      simpl in Hceval. simpl.
      destruct (ceval_step st c1) eqn: Heqst1'o.
      ---(*success = success *)
        induction s.
        apply (IHi1' i2') in Heqst1'o; try assumption.
        rewrite -> Heqst1'o. rewrite Hceval.
        reflexivity.
      ---(* success = fail *)
        simpl in Hceval. discriminate Hceval.
      ---(* success = OutOfGas*)
    ...
    + (* -> *)
      simpl in Hceval. simpl.
      destruct (beval st b) eqn: Heqst1'o.
      ---(*  *)
        apply (IHi1' i2') in Hceval; try assumption.
      --- destruct cont; try discriminate.
      ----- destruct p.  apply (IHi1' i2') in Hceval; try assumption.
  Qed.
